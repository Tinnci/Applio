...
# from core import run_infer_script, run_batch_infer_script # No longer calling these directly
from core import import_voice_converter # Import the function that imports the class
...
class InferenceWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # --- Actual Call using self.converter ---
            if self.is_batch:
                 # Call converter's batch method
                 message = self.converter.convert_audio_batch(**core_params)
                 output_path = self.params.get("output_folder")
            else:
                 # Call converter's single method
                 message, output_path = self.converter.convert_audio(**core_params)
            # --- End Actual Call ---

            # Check if cancelled during conversion
            if not self._is_running:
                self.status.emit(f"{task_name} cancelled.")
                print(f"{task_name} process was cancelled during execution.")
                return # Exit run method early

            # Fake progress for now, replace with real progress later if possible
    ...
    def cancel_process(self):
        """Requests cancellation of the running inference task."""
        print("Requesting cancellation from InferenceWorker...")
        self._is_running = False # Set internal flag
        if self.converter:
            self.converter.request_cancellation() # Call converter's cancellation method
...
class InferenceTab(QWidget):
    ...
    def setup_ui(self):
        ...
        # Left side: Convert button, Status, Progress
        left_v_layout = QVBoxLayout()
        self.convert_button = QPushButton("Convert")
        self.convert_button.clicked.connect(self.start_inference)
        left_v_layout.addWidget(self.convert_button)
        ...
        action_status_layout.addLayout(left_v_layout) # Add the vertical layout to the horizontal one

        # Right side: Cancel button
        self.cancel_button_infer = QPushButton("Cancel")
        self.cancel_button_infer.setEnabled(False)
        self.cancel_button_infer.clicked.connect(self.request_cancellation_infer)
        action_status_layout.addWidget(self.cancel_button_infer) # Add cancel button next to the vertical layout
    ...
    def start_inference(self):
        ...
        if self.inference_worker and self.inference_worker.isRunning():
            ...
            if reply == QMessageBox.StandardButton.Yes and self.inference_worker:
                self.inference_worker.cancel_process() # Use cancel_process instead of stop()
                # Optionally wait briefly for thread to finish before starting new one
                # self.inference_worker.wait(1000) # Wait up to 1 second
            else:
                return
        ...
        self.inference_worker.finished.connect(lambda msg, path: self.cancel_button_infer.setEnabled(False))
        self.inference_worker.error.connect(lambda err_msg: self.cancel_button_infer.setEnabled(False))
        self.cancel_button_infer.setEnabled(True) # Enable cancel button
        self.inference_worker.start()

    def request_cancellation_infer(self):
        """Sends cancellation request to the inference worker thread."""
        if self.inference_worker and self.inference_worker.isRunning():
            print("Cancel button clicked, requesting cancellation from inference worker...")
            self.inference_worker.cancel_process()
            self.cancel_button_infer.setEnabled(False) # Disable after requesting
            self.status_label.setText("Status: Cancellation requested...")
        else:
            print("Cancel button clicked, but no inference worker is running.")

    def reset_ui_state(self):
        self.convert_button.setEnabled(True)
        self.progress_bar.setValue(0)
        self.progress_bar.setTextVisible(False)
        self.cancel_button_infer.setEnabled(False) # Ensure cancel is disabled on reset
